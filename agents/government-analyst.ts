...
export default defineAgent({
  ...
  version: '0.1.0',
  description: 'Analyst for Spanish municipal government meeting transcriptions. Extracts session ID, detects follow-ups, generates WhatsApp-formatted summaries, and stores them in Struere.',
model: {
...
3. If is_followup=true, retrieve prior meeting context using meetings_get_summary with the prior session_id
...
6. Save the summary using meetings_save_summary, passing the source URL
7. Return ONLY the JSON response { "id": "<entity_id>" } — no additional text

OUTPUT FORMAT: Strict JSON only { id: string }
Example: { "id": "jh7adx98c6qht9qrsn36kt37m986w6zn" }

IMPORTANT:
- Do NOT apply any markdown formatting to the JSON output, strict plain JSON.
